...
2. Copy the plugin to Airflow's plugins folder:
   ...
   **In Breeze**, that folder is `files/plugins/` — every compose variant sets
   `AIRFLOW__CORE__PLUGINS_FOLDER=/files/plugins`, and this source directory is
   *not* mounted into the container. Re-run this after every edit or the
   container keeps serving the previous copy:
